...
* `--blocks_to_swap=<integer>`
  ...
  - Cannot be used with `--cpu_offload_checkpointing`, `--unsloth_offload_checkpointing`, or `--deepspeed`.
* `--unsloth_offload_checkpointing`
  - Offload activations to CPU RAM using async non-blocking transfers (faster than `--cpu_offload_checkpointing`). Cannot be combined with `--cpu_offload_checkpointing`, `--blocks_to_swap`, or `--deepspeed`.
* `--cache_text_encoder_outputs`
...
#### DeepSpeed Notes / DeepSpeed 注意点

* DeepSpeed can be used for Anima LoRA training. With ZeRO-3, LoRA weights are saved from the consolidated DeepSpeed state dict.
* `--mixed_precision=bf16` with 8-bit optimizers is automatically changed to a supported non-8bit optimizer by `library/deepspeed_utils.py`.
* Do not combine DeepSpeed with `--blocks_to_swap` or `--unsloth_offload_checkpointing`; those memory paths are not wired through the DeepSpeed engine for Anima LoRA.
...
* `--blocks_to_swap` - TransformerブロックをCPUとGPUでスワップしてVRAMを節約。`--cpu_offload_checkpointing`、`--unsloth_offload_checkpointing`、`--deepspeed`とは併用できません。
* `--unsloth_offload_checkpointing` - 非同期転送でアクティベーションをCPU RAMにオフロード。`--cpu_offload_checkpointing`、`--blocks_to_swap`、`--deepspeed`とは併用できません。
...
- **`--blocks_to_swap <number>`**: Swaps blocks between CPU and GPU to reduce VRAM usage. Higher numbers save more VRAM but reduce training speed. See model-specific max values in section 4.1. Cannot be combined with `--deepspeed`.

- **`--unsloth_offload_checkpointing`**: Offloads gradient checkpoints to CPU using async non-blocking transfers. Faster than `--cpu_offload_checkpointing`. Cannot be combined with `--blocks_to_swap` or `--deepspeed`.
...
This script extracts an approximated LoRA from the difference between two Anima models. By default it extracts DiT LoRA only; if you also specify `--qwen3_org` and `--qwen3_tuned`, it can extract Qwen3 text encoder LoRA at the same time.
...
Add `--train_llm_adapter` if you also want LoRA extraction for LLM Adapter blocks.
...
2つのAnimaモデルの差分から近似LoRAを抽出するスクリプトです。デフォルトではDiTのLoRAのみを抽出します。`--qwen3_org`と`--qwen3_tuned`も指定すると、Qwen3テキストエンコーダーのLoRAも同時に抽出できます。
...
LLM Adapterブロックも抽出したい場合は、`--train_llm_adapter`を追加してください。

</details>

### `networks/anima_merge_lora.py`

This script merges one or more Anima LoRA files into an Anima DiT model and saves a new `.safetensors` model.
...
Multiple LoRA files can be passed to `--models` with matching `--ratios`. If a LoRA file contains Qwen3 text encoder weights, those tensors are skipped; only the DiT part is merged into the Anima DiT model.
...
1つまたは複数のAnima LoRAをAnima DiTモデルへマージし、新しい`.safetensors`モデルとして保存するスクリプトです。
...
複数のLoRAを使う場合は、`--models`と対応する`--ratios`を同じ順番で指定します。LoRAにQwen3テキストエンコーダーの重みが含まれている場合、それらはスキップされ、DiT部分だけがAnima DiTモデルにマージされます。
